...
OPTIMAL_TRAINING_WINDOW_BARS = 4000  # from walk-forward backtest, see backend/data/backtest_report.json
...
def build_feature_row(df_ind, i: int) -> dict:
    ...
    Index i represents the interval being predicted, and features are drawn from completed
    bars prior to i.
    """
    ...
    c_high = float(c.get("high", max(c_open, actual_close)))
    c_low = float(c.get("low", min(c_open, actual_close)))
    rng = max(1e-5, c_high - c_low)

    upper_wick = (c_high - max(c_open, actual_close)) / rng
    lower_wick = (min(c_open, actual_close) - c_low) / rng
    body_range = abs(actual_close - c_open) / rng
    ...
    range_24h_pos = (actual_close - low_24h_val) / max(1.0, high_24h_val - low_24h_val)
...
def build_live_ml_features(
    ...
    c_close = float(c.get("close", 60000.0) or 60000.0)
    c_open = float(c.get("open", c_close) or c_close)
    c_high = float(c.get("high", max(c_open, c_close)) or max(c_open, c_close))
    c_low = float(c.get("low", min(c_open, c_close)) or min(c_open, c_close))

    target_price = float(target) if target is not None else float(c_open)
    rng = max(1e-5, c_high - c_low)

    upper_wick = (c_high - max(c_open, c_close)) / rng
    lower_wick = (min(c_open, c_close) - c_low) / rng
    body_range = abs(c_close - c_open) / rng

    high_24h_val = float(df_ind["high"].tail(96).max()) if len(df_ind) > 0 and "high" in df_ind.columns else c_close
    low_24h_val = float(df_ind["low"].tail(96).min()) if len(df_ind) > 0 and "low" in df_ind.columns else c_close
    vol_24h_val = float(df_ind["volume"].tail(96).sum()) if len(df_ind) > 0 and "volume" in df_ind.columns else 0.0
    range_24h_pos = (c_close - low_24h_val) / max(1.0, high_24h_val - low_24h_val)
    range_24h_pos = max(0.0, min(1.0, range_24h_pos))

    delta_to_target = float(((c_close - target_price) / max(target_price, 1e-9)) * 100.0)

    rsi = float(c.get("rsi", 50.0) or 50.0)
    ema_9 = float(c.get("ema_9", c_close) or c_close)
    ema_21 = float(c.get("ema_21", c_close) or c_close)
    ema_50 = float(c.get("ema_50", c_close) or c_close)
    bb_upper = float(c.get("bb_upper", c_high) or c_high)
    bb_lower = float(c.get("bb_lower", c_low) or c_low)
    atr = float(c.get("atr", 100.0) or 100.0)
    ...
    cvd_val = float(df_ind["cvd"].iloc[-1]) if len(df_ind) > 0 and "cvd" in df_ind.columns else 0.0

    vwap_val = c.get("vwap")
    price_vs_vwap = float(c_close - vwap_val) if vwap_val is not None and not pd.isna(vwap_val) else 0.0
    ...
    try:
        ...
        avg_vol_3d = float(vol_slice.mean()) if len(vol_slice) > 0 else 1.0
        curr_vol = float(c.get("volume", 0.0))
        vol_15m_ratio = curr_vol / max(avg_vol_3d, 1e-9)
    except Exception:
        vol_15m_ratio = 1.0
    ...
    features = {
        ...
        "news_sentiment_score": float(c.get("news_sentiment_score", 0.0) or 0.0),
        ...
        "roc_15m": float(df_ind["roc_15m"].iloc[-1]) if "roc_15m" in df_ind.columns and len(df_ind) > 0 else 0.0,
        "roc_1h": float(df_ind["roc_1h"].iloc[-1]) if "roc_1h" in df_ind.columns and len(df_ind) > 0 else 0.0,
        "roc_4h": float(df_ind["roc_4h"].iloc[-1]) if "roc_4h" in df_ind.columns and len(df_ind) > 0 else 0.0,
...
class MLEngine:
    ...
    def __init__(self, data_dir, trading_style="SNIPER"):
        ...
        if self.trading_style == "MACHINE_GUN":
            self.feature_keys.append("minutes_remaining")
        self._lock = threading.Lock()
        self.train_window = 15000 if self.trading_style == "MACHINE_GUN" else 100
...
def get_ml_engine(data_dir: str = None, trading_style: str = "SNIPER"):
    global _ml_engine_instances
    if trading_style not in _ml_engine_instances:
        with _ml_engine_lock:
            if trading_style not in _ml_engine_instances:
                if data_dir is None:
                    data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data")
                from backend.btc.dual_ml_engine import DualMLEngine
                engine = DualMLEngine(data_dir, trading_style)
                engine.train()
                _ml_engine_instances[trading_style] = engine
    return _ml_engine_instances[trading_style]
