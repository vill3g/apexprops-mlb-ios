...
for g, ts in sorted(grades.items()):
    ...
    print(f"  {g:<30} {w}/{len(ts)} = {round(w/len(ts)*100,1)}%  PNL=\")
print()
...
for d, ts in sorted(dirs.items()):
    w = sum(1 for t in ts if t.get('result')=='WIN')
    pnl = sum(float(t.get('pnl',0)) for t in ts)
    print(f"  {d:<10} {w}/{len(ts)} = {round(w/len(ts)*100,1)}%  PNL=\")
print()
...
for s, ts in sorted(srcs.items()):
    w = sum(1 for t in ts if t.get('result')=='WIN')
    pnl = sum(float(t.get('pnl',0)) for t in ts)
    print(f"  {s:<25} {w}/{len(ts)} = {round(w/len(ts)*100,1)}%  PNL=\")
print()
...
for h in sorted(hours.keys()):
    ...
    pnl = sum(float(t.get('pnl',0)) for t in ts)
    bar = '#' * w + '.' * (len(ts)-w)
    print(f"  {h:02d}:00  {w}/{len(ts)} = {round(w/len(ts)*100,1)}%  PNL=\  [{bar}]")
print()
...
if agree:
    ...
    print(f"  ML + Tech AGREE  : {aw}/{len(agree)} = {round(aw/len(agree)*100,1)}%  PNL=\")
if conflict:
    cw = sum(1 for t in conflict if t.get('result')=='WIN')
    print(f"  ML vs Tech CONFLICT: {cw}/{len(conflict)} = {round(cw/len(conflict)*100,1)}%  PNL=\")
print()

# --- Biggest wins and losses ---
print("=== TOP 5 WINS ===")
for t in sorted(live, key=lambda x: float(x.get('pnl',0)), reverse=True)[:5]:
    print(f"  \  {t.get('conviction_grade')}  {t.get('direction')}  {t.get('timestamp','')[:16]}")
print()
print("=== TOP 5 LOSSES ===")
