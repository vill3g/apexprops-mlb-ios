...
for i, line in enumerate(lines):
    if line.strip().startswith('def execute_for_user(user):'):
        skip = True
        
        # Insert properly formatted block
...
"""
    ...
    if skip:
        ...
        if "logger.error(f'[SaaS Broadcast] Error executing for user" in line or 'logger.error(f"[SaaS Broadcast] Error executing for user' in line:
            skip = False
        continue
